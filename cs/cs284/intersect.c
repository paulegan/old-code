...
/* intersect.c - A program that forms the intersection of two binary files based on 	*/
...
void fileIntersection(char *inFile1_name,char *inFile2_name,char *outFile_name);
...
void main(int argc, char *argv[])
{
 ...
 if ( !recordCompatible(inFile1_name,inFile2_name) )
  ...
  fileIntersection(inFile1_name,inFile2_name,outFile_name);
...
void fileIntersection(char *inFile1_name,char *inFile2_name,char *outFile_name)
{					/* Outputs the intersection of inFile1 and	*/
 FILE *inFile1;				/*  inFile2 to outFile.				*/
 FILE *inFile2;
 FILE *outFile;
 ...
 if ((outFile=fopen(outFile_name,"wb"))==NULL){
  ...
  exit(1);
					...
					/* Copies everything in inFile1 that also	*/
 while(!feof(inFile1)){			/*  exists in inFile2 to outFile		*/
  fread(record,recordLength,1,inFile1); 
  if (exist(inFile2,record,recordLength))
